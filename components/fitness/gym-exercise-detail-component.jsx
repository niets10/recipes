...
export function GymExerciseDetailComponent({ exercise }) {
    ...
    if (!exercise) {
        return <div className="text-muted-foreground">Exercise not found.</div>;
    }
            ...
            <Card className="border-t-4 fitness-card-border">
                <CardHeader className="flex flex-row items-start justify-between gap-4">
                    <div className="flex min-w-0 flex-1 items-start gap-2">
                        <BackLink label="Back to gym exercises" />
                ...
                <CardContent className="space-y-2 text-sm">
                    ...
                    {exercise.comments && (
                        <p className="text-muted-foreground italic">{exercise.comments}</p>
                    )}
                </CardContent>
            </Card>
        </div>
